...
def calculate_rmsd_obrms(pdb_file, sdf_file, output_dir, software_name):
    """
    使用 obrms 计算配体参考结构（PDB）和对接工具结果（SDF）的 RMSD，按软件标识生成结果文件，并返回RMSD。
    """
    pdbname = os.path.splitext(os.path.basename(pdb_file))[0]
    rmsd_csv = os.path.join(output_dir, f"{pdbname}_align_{software_name}.csv")

    try:
        # 打开文件写入 RMSD 结果
        with open(rmsd_csv, 'w') as output_file:
            result = subprocess.run(['obrms', pdb_file, sdf_file], check=True, capture_output=True, text=True)
        ...
        for line in result.stdout.splitlines():
            if "RMSD" in line:
                rmsd_value = float(line.split()[-1])
                break
        
        logging.info(f"RMSD calculation completed: {rmsd_csv}, RMSD: {rmsd_value}")
        return {"method": software_name, "rmsd": rmsd_value}
    
    except subprocess.CalledProcessError as e:
        logging.error(f"Error in RMSD calculation using obrms: {e}")
        return None

def calculate_rmsd_ledock(pdb_file, mol2_file, output_dir, software_name):
    """
    使用 obrms 计算 LeDock 对接结果（MOL2）与参考结构（PDB）的 RMSD，并返回结果。
    """
    pdbname = os.path.splitext(os.path.basename(pdb_file))[0]
    rmsd_csv = os.path.join(output_dir, f"{pdbname}_align_{software_name}.csv")
    ...
    try:
        # 打开文件写入 RMSD 结果
        with open(rmsd_csv, 'w') as output_file:
            result = subprocess.run(['obrms', pdb_file, mol2_file], check=True, capture_output=True, text=True)
            output_file.write(result.stdout)
        ...
        logging.info(f"LeDock RMSD calculation completed: {rmsd_csv}, RMSD: {rmsd_value}")
        return {"method": software_name, "rmsd": rmsd_value}
    
    except subprocess.CalledProcessError as e:
        logging.error(f"Error in RMSD calculation for LeDock: {e}")
        return None
...
def extract_rmsd_karmadock(karmadock_csv):
    """
    从 KarmaDock 的 CSV 文件中提取 RMSD 结果。
    
    :param karmadock_csv: KarmaDock 的 CSV 文件路径
    :return: 返回一个包含所有 KarmaDock 方法和 RMSD 的列表
    """
    rmsd_results = []
    try:
        with open(karmadock_csv, 'r') as csvfile:
            reader = csv.DictReader(csvfile)
            for row in reader:
                rmsd_results.append({"method": row["Method"], "rmsd": float(row["RMSD"])})
    except FileNotFoundError:
        logging.error(f"File not found: {karmadock_csv}")
    return rmsd_results

def combine_rmsd_results(pdbname, rmsd_results, output_csv):
    """
    合并 RMSD 结果并保存到新的 CSV 文件中。
    
    :param pdbname: PDB 名称
    :param rmsd_results: 合并的 RMSD 结果列表
    ...
    with open(output_csv, 'a', newline='') as csvfile:
        ...
        if not file_exists:
            # 写入表头
            writer.writerow(["PDB_ID", "Method", "RMSD"])
        
        for result in rmsd_results:
            writer.writerow([pdbname, result["method"], result["rmsd"]])
...
def perform_docking(protein_pdb, center_csv, protein_pdb_dir, base_output_dir, pdbqt_output_dir, mol2_output_dir, smiles_csv, mgltool_path, autodock_gpu_path, karmadock_path, ledock_path, ligand_pdb, pdbname):
    ...
    # Convert .dok to .mol2 using dok2mol2, with output redirection
    dok2mol2_cmd = os.path.join(ledock_path, "dok2mol2")
    dok2mol2_output = os.path.join(ledock_output_dir, f"{pdbname}_lig.mol2")  
    template_mol2 = os.path.join(base_output_dir, f"{pdbname}_lig.mol2")  

    if os.path.exists(ledock_dok) and os.path.exists(template_mol2):
        try:
            # Execute dok2mol2 to convert .dok to .mol2 and redirect output to the final .mol2 file
            with open(dok2mol2_output, 'w') as mol2_file:
                subprocess.run([dok2mol2_cmd, ledock_dok, template_mol2, '1'], check=True, stdout=mol2_file)
            logging.info(f"Converted {ledock_dok} to {dok2mol2_output} using dok2mol2")
        except subprocess.CalledProcessError as e:
            logging.error(f"Error in converting .dok to .mol2 using dok2mol2: {e}")
    else:
        logging.error(f".dok file or template .mol2 file not found: {ledock_dok}, {template_mol2}")
...
def process_protein(pdb_file, redock_dir, mgltool_path, autodock_gpu_path, karmadock_path, ledock_path):
    ...
    perform_docking(
        ...
        protein_pdb_dir=os.path.dirname(protein_chain_file),
        base_output_dir=split_data_dir,
        pdbqt_output_dir=split_data_dir,
        mol2_output_dir=split_data_dir,
    ...
    # 固定使用 {pdbname}_lig.pdb 作为参考配体
    reference_pdb = os.path.join(split_data_dir, f"{pdbname}_lig.pdb")
    if not os.path.exists(reference_pdb):
        print(f"Reference PDB file not found: {reference_pdb}")
        return

    docking_software_dirs = {
        "AutodockGPU": os.path.join(split_data_dir, f"{pdbname}_chain_AutodockGPU"),
        "Vina": os.path.join(split_data_dir, f"{pdbname}_chain_Vina"),
        "LeDock": os.path.join(split_data_dir, f"{pdbname}_chain_ledock")
    }

    rmsd_results = []

    # AutodockGPU and Vina RMSD Calculation (using SDF)
    for software, path in docking_software_dirs.items():
        if software != "LeDock":  # Exclude LeDock here, handle it separately
            sdf_file = os.path.join(path, f"{pdbname}_lig.sdf")

            if os.path.exists(sdf_file):
                print(f"Calculating RMSD for {software} using {sdf_file} ...")
                rmsd_result = calculate_rmsd_obrms(reference_pdb, sdf_file, split_data_dir, software)
                if rmsd_result:
                    rmsd_results.append(rmsd_result)
            else:
                print(f"File not found: {sdf_file}")

    # LeDock RMSD Calculation (using MOL2)
    ledock_mol2 = os.path.join(path, f"{pdbname}_lig.mol2")
    if os.path.exists(ledock_mol2):
        print(f"Calculating RMSD for LeDock using {ledock_mol2} ...")
        rmsd_result = calculate_rmsd_ledock(reference_pdb, ledock_mol2, split_data_dir, "LeDock")
        if rmsd_result:
            rmsd_results.append(rmsd_result)
    else:
        print(f"LeDock MOL2 file not found: {ledock_mol2}")

    # 提取 KarmaDock 的 RMSD
    karmadock_csv = os.path.join(split_data_dir, f"{pdbname}_chain_KarmaDock", f"{pdbname}_karmadock_rmsd.csv")
    rmsd_results.extend(extract_rmsd_karmadock(karmadock_csv))

    # 合并结果并保存到新的 CSV
    output_csv = os.path.join(redock_dir, "combined_rmsd_results.csv")
    combine_rmsd_results(pdbname, rmsd_results, output_csv)
