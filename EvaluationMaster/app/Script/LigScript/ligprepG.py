...
OUT_MAE   {os.path.splitext(csv_filename)[0]}-out.maegz
MAX_ATOMS   500
FORCE_FIELD   16
EPIK   yes
...
def process_molecules(njobs, input_file, save_dir, Schro_dir):
    ...
    if file_ext == '.smi':
        csv_filename = convert_file_to_csv(input_file)
    elif file_ext == '.csv':
        csv_filename = process_csv_file(input_file)
    else:
        raise ValueError(f"Unsupported file format: {file_ext}")

    # Ensure the working directory exists
    if not os.path.exists(save_dir):
        # raise FileNotFoundError(f"Specified working directory '{working_directory}' does not exist.")
    ...
    # Create a subdirectory named after the CSV file
    if not os.path.exists(subdir_path):
        os.makedirs(subdir_path)

    # Copy the CSV file to the new subdirectory
    csv_destination = os.path.join(subdir_path, csv_basename)
    shutil.copy(csv_filename, csv_destination)
    print(f"CSV file copied to: {csv_destination}")

    # Generate and save the INP file
    inp_filepath = os.path.join(subdir_path, subdir_name + '.inp')
    ...
    sh_filepath = os.path.join(subdir_path, subdir_name + '.sh')
