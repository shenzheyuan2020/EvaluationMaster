...
def process_csv_after_glide_adjust(mol_name, base_dir):
    ...
    # 构建Excel文件路径
    excel_file_path = os.path.join(base_dir, f"results.csv")
    # 保存去重并排序后的数据到Excel
    final_sorted_data.to_csv(excel_file_path, index=False)
    # 打印保存信息
...
def map_and_merge(save_dir, result_csv_dir):
    # 读取CSV文件
    results_csv_path = os.path.join(result_csv_dir, "results.csv")
    ref_csv_path = os.path.join(save_dir, "..", "ref_standard.csv")
    results_df = pd.read_csv(results_csv_path)
    ref_df = pd.read_csv(ref_csv_path)
    
    # 使用左连接保留results_df中的条目，并附加ref_df中的信息
    merged_df = pd.merge(results_df, ref_df, on='Name', how='left')
    
    # 确保smiles列存在于DataFrame中
    if 'smiles' in merged_df.columns:
        # 将smiles列移动到第一列
        cols = ['smiles'] + [col for col in merged_df.columns if col != 'smiles']
        merged_df = merged_df[cols]
    else:
        print("Warning: 'smiles' column not found in merged DataFrame.")

    # 构建输出文件路径
    output_csv_path = os.path.join(save_dir, "results.csv")
    
    # 保存合并后的数据到CSV
    merged_df.to_csv(output_csv_path, index=False)
    print(f"Mapped and merged data saved to {output_csv_path} with 'smiles' as the first column")
...
def process_glide_csv(csv_file_path, lig_path, schro_dir, mm_sahre_dir, save_dir, Glide_mode, threads_num):
    with open(csv_file_path, newline='') as csvfile:
        ...
        for row in reader:
            ...
            map_and_merge(save_dir, os.path.join(save_dir, mol_name))
def main():
