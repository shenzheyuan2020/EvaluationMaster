...
class VSTestGraphDataset_FlyReload_CSV(VSTestGraphDataset_Fly):
    '''Initialize the ligand pose with RDKit from molecules defined by SMILES in a CSV file'''
    def __init__(self, protein_file, ligand_csv, pocket_center, graph_dir):
        super().__init__(protein_file, ligand_csv, pocket_center)
        self.ligand_data = pd.read_csv(ligand_csv)
        self.ligand_names = self.ligand_data['Name'].tolist()
        self.ligand_smis = self.ligand_data['smiles'].tolist()
        self.graph_dir = graph_dir  # Assumes CSV file is in the directory where graphs should be saved

    def generate_graphs(self, n_job=-1, verbose=True):
        if n_job == 1:
            if verbose:
                iters = tqdm(range(len(self.ligand_names)))
            else:
...
if __name__ == '__main__':
    lig_mol2 = '/home/hoo/Desktop/Zheyuan/VM/Support_software/KarmaDock/test/5BVO_VS/5BVO_crystal_ligand.mol2'
    xyz = get_mol2_xyz_from_cmd(lig_mol2)
    print(xyz)
